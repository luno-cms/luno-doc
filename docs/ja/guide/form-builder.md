...
::: tip 今すぐやる（3 行）
1. **設定 → フォームセット → 新規作成**（表示名と slug）  
2. 必須フィールドを 2〜3 個足して保存する  
3. エントリを 1 件作り、公開 API の `/form-sets/{slug}/entries` を叩く  
:::
...
概要・サイト公開・公開 API は [Masters](/ja/products/masters) を参照してください。
