---
title: LUNO Documentation
description: User guide, public API reference, and AI agent guide for LUNO.
layout: home

hero:
  name: LUNO
  text: Documentation
  tagline: From content creation and management to API integration and AI agent usage.
  actions:
    - theme: brand
      text: Getting Started
      link: /en/guide/getting-started
    - theme: alt
      text: Public API Reference
      link: /en/api/public-api

features:
  - icon: 📝
    title: Content Management
    details: Approval workflow (draft → review → publish), scheduled publishing, revision diffs, and assignee support.
  - icon: 🔧
    title: Form Builder
    details: Design your content model with 11 field types — text, image, rich text, entry references, and more.
  - icon: 🌐
    title: Public API (No Auth Required)
    details: Fetch entries, media, and more without authentication. Works directly from any frontend or Jamstack site.
  - icon: 🤖
    title: AI Agent Ready
    details: Built-in llms.txt and llms-full.txt endpoints let AI agents autonomously discover API specs and content structure.
  - icon: 🪝
    title: Webhooks
    details: Deliver entry publish/update/delete events with HMAC-SHA256 signatures for real-time external integrations.
  - icon: 📬
    title: Contact Forms
    details: Create forms, manage submissions, and send notifications to Slack, Teams, LINE Notify, and more.
---
